...
import { Mic, X, ArrowRight, Paperclip, Search, MicOff } from "lucide-react";
...
const Listening = () => {
  ...
  useEffect(() => {
    ...
    if (recognition) {
      ...
      // Inicia ouvindo automaticamente
      handleToggleListen();
    }
    ...
    <div className="min-h-[80vh] flex flex-col px-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
      <div className="pt-8 text-center">
        <h2 className="text-3xl font-extrabold text-[#1B4332]">
          {isListening ? "Estou ouvindo..." : "Toque no microfone"}
        </h2>
        <p className="text-gray-500 font-medium mt-1">Diga sua dúvida sobre o e-SUS ou o campo.</p>
      </div>

      <div className="flex-1 flex flex-col items-center justify-center gap-8">
        <button 
          onClick={handleToggleListen}
          className={`w-32 h-32 rounded-full flex items-center justify-center transition-all duration-500 ${isListening ? 'bg-emerald-100 scale-110' : 'bg-[#B7E4C7]'}`}
        >
          <div className={`w-24 h-24 rounded-full flex items-center justify-center shadow-lg transition-all ${isListening ? 'bg-red-500 animate-pulse' : 'bg-[#1B4332]'}`}>
